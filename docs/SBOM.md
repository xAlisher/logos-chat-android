...
|---|---|---|
| `liblogoschat.so` | MLS chat core (E2E messaging) | Published by `xAlisher/logos-libchat-mls-android` @ `0b6c0ca`; **patch-based** on pinned upstream `libchat` commit `d2124fd` + `patches/libchat-android-arm64.patch` + `patches/349-groupv1-remove-member.patch` + `patches/437-replace-on-desync-welcome.patch` + `patches/433-expose-group-creator.patch` + `patches/490-keypackage-device-binding.patch` + `patches/491-keypackage-card-binding.patch` |
...
build: `069cdb3d…` in `jniLibs/` = `069cdb3d…` in `lib/arm64-v8a/` of `app-release.apk`).
...
- `xAlisher/logos-libchat-mls-android@04bc30d` **published the exact artifact** this app
  ...
  since — see rounds 3 and 4 below; the shipped binary is now `069cdb3d…` @ `0b6c0ca`.)
...
- Source pins: upstream `libchat` @ `d2124fd` + the `logos-libchat-mls-android` @ `0b6c0ca`
